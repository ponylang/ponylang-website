...
If you know what you are doing, Pony can make it easy to write high-performance code. Pony isn't, however, a performance panacea. There are plenty of things that you a programmer can do to hurt performance.
...
The easiest way to end up boxing a machine word is by including it in a union type with non-machine words. Take a look at the following code:
...
You probably won't be proud of that code, but you'll be proud of the performance improvement you get. The `-1` idiom is one that should be quite familiar to folks with a C background. If you aren't familiar with C, you might be thinking: "Wait, that's a `USize`, an unsigned value. What on earth is `-1` there?" And that's a good question to ask. The answer is numeric types wrap overflow and wrap around. So `-1` is equivalent to the max value of a `U64`. Keep that in mind because if you find your value in index `9,223,372,036,854,775,807`, you'll be treating it as not-found. Probably not an issue but, ¯\_(ツ)_/¯ you never know.
