...
ponyc can be built from source with instrumentation options for debugging: Valgrind annotations, sanitizers (address, thread, undefined behavior), and systematic testing for the runtime's own concurrency bugs. See [Custom ponyc Builds for Debugging](compiler/custom-ponyc-builds.md).
